...
public class InventoryExamineGuiProvider implements ExamineGuiItemProvider {
    ...
    public boolean enabled(Player staff, SppPlayer player) {
        if (player.isOnline()) {
            return permissionHandler.has(staff, examineConfiguration.permissionExamineViewInventory);
        } else {
            return permissionHandler.has(staff, examineConfiguration.permissionExamineViewInventoryOffline);
        }
    }
