...
public class IpBanBungeeListener implements PluginMessageListener {
    ...
    public IpBanBungeeListener(BungeeClient bungeeClient, ServerSyncConfiguration serverSyncConfiguration) {
    ...
    public void onPluginMessageReceived(String channel, Player player, byte[] message) {
        ...
        if (banBungeeDto.isPresent() && serverSyncConfiguration.banSyncServers.matchesServer(banBungeeDto.get().getServerName())) {
            Bukkit.getPluginManager().callEvent(new IpBanBungeeEvent(banBungeeDto.get()));
        }
    }
}
