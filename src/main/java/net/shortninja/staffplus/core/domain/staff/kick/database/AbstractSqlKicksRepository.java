...
public abstract class AbstractSqlKicksRepository implements KicksRepository {
    ...
    public AbstractSqlKicksRepository(PlayerManager playerManager, SqlConnectionProvider sqlConnectionProvider, Options options) {
        ...
        serverNameFilter = !options.serverSyncConfiguration.kickSyncServers.isEnabled() ? "AND (server_name is null OR server_name='" + options.serverName + "')" : "";
    }
    ...
    public Map<UUID, Integer> getCountByPlayer() {
        ...
        try (Connection sql = getConnection();
             PreparedStatement ps = sql.prepareStatement("SELECT player_uuid, count(*) as count FROM sp_kicked_players " + Constants.getServerNameFilterWithWhere(options.serverSyncConfiguration.kickSyncServers) + " GROUP BY player_uuid ORDER BY count DESC")) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
