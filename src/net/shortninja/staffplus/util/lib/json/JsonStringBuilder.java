...
public class JsonStringBuilder
{
	...
	{
		message = msg;
		string = ",{\"text\":\"" + pluginMessage.colorize(text) + "\"";
	}
	
	private String esc(String s)
	{
		return JSONObject.escape(pluginMessage.colorize(s));
	}
	...
	public JsonStringBuilder setHoverAsTooltip(String ... lore)
	{
		...
		for(int i = 0; i < lore.length; i++)
		{
			if(i + 1 == lore.length)
			{
				builder.append(pluginMessage.colorize(lore[i]));
			}else builder.append(pluginMessage.colorize(lore[i]) + "\n");
		}
	...
	{
		message.msg += pluginMessage.colorize(string + hover + click + "}");
		return message;
	}
}