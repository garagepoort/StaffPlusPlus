...
public class UpdateNotifier extends BukkitRunnable {
    ...
    public UpdateNotifier() {
        ...
        runTaskAsynchronously(TubingBukkitPlugin.getPlugin());
    }
            ...
            spigetClient.getVersions().enqueue(new Callback<JsonObject>() {
                ...
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if (response.isSuccessful()) {
                        String currentVersion = TubingBukkitPlugin.getPlugin().getDescription().getVersion();

                        String jar = response.body().get("1.18").getAsString();
                        if (jar == null) {
                            return;
                        }
