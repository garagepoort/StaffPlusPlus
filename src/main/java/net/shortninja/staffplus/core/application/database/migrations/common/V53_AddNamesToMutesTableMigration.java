...
public class V53_AddNamesToMutesTableMigration implements Migration {
    ...
    public List<String> getStatements() {
        ...
        List<String> updates = Arrays.stream(Bukkit.getOfflinePlayers()).flatMap(p -> Stream.of(
            String.format("UPDATE sp_muted_players set player_name='%s' WHERE player_uuid='%s';", p.getName(), p.getUniqueId()),
            String.format("UPDATE sp_muted_players set issuer_name='%s' WHERE issuer_uuid='%s';", p.getName(), p.getUniqueId())))
