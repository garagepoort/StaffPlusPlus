...
<#macro evidenceButton slot evidence>
    <#if $config.get("investigations-module.enabled") && $permissions.has(player, $config.get("permissions:investigations.manage.link-evidence"))>
        <GuiItem slot="${slot}"
                 ...
                 onLeftClick="manage-investigation-evidence/view/investigation-link?evidenceId=${evidence.id}&evidenceType=${evidence.evidenceType}&evidenceDescription=${evidence.description}"
                 material="ANVIL">
