...
public class ModeDataSerializer {
    ...
    private File createFile(String uuid) {
        ...
        File folder = new File(TubingBukkitPlugin.getPlugin().getDataFolder(), "ModeData");
        if (!folder.exists()) {
            folder.mkdir();
        }
        if (!file.exists()) {
    ...
    private File getFile(String uuid) {
        return new File(TubingBukkitPlugin.getPlugin().getDataFolder() + File.separator + "ModeData" + File.separator + uuid + ".yml");
    }
}
