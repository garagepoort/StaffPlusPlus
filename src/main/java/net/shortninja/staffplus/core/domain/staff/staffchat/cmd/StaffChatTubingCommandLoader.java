...
public class StaffChatTubingCommandLoader {
    ...
    public static List<SppCommand> loadCommands(Messages messages, StaffChatConfiguration staffChatConfiguration, SessionManagerImpl sessionManager, StaffChatServiceImpl staffChatService, CommandService commandService, PermissionHandler permissionHandler) {
        ...
        for (StaffChatChannelConfiguration channelConfiguration : channelConfigurations) {
            commands.add(new StaffChatChannelCmd(messages, sessionManager, staffChatService, commandService, channelConfiguration, permissionHandler));
            commands.add(new StaffChatMuteChannelCmd(messages, sessionManager, commandService, channelConfiguration, permissionHandler));
        }
        return commands;
    }
