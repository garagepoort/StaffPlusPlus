...
public class PlayerSettingsDataFile {
    ...
    public PlayerSettingsDataFile(@InjectTubingPlugin TubingBukkitPlugin tubingPlugin) {
        this.tubingPlugin = tubingPlugin;
        File file = new File(tubingPlugin.getDataFolder(), DATA_YML);
        if (!file.exists()) {
            tubingPlugin.saveResource(DATA_YML, false);
        }
