...
public abstract class AbstractSqlMuteRepository implements MuteRepository {
    ...
    public List<Mute> getMyMutes(UUID playerUuid, int offset, int amount) {
        List<Mute> mutes = new ArrayList<>();
        try (Connection sql = getConnection();
             PreparedStatement ps = sql.prepareStatement("SELECT * FROM sp_muted_players WHERE player_uuid = ? AND soft_mute = ?" + getServerNameFilterWithAnd(muteSyncServers) + " ORDER BY creation_timestamp DESC LIMIT ?,?")) {
            ps.setString(1, playerUuid.toString());
